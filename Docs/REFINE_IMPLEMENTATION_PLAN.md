...
- ステータス: R0〜R6 完了。REFINE_PLAN.md の 8 項目はすべて対応済み。残りは §10 の積み残し(実機での fps 計測)
...
実施結果: 等距離視点で足りた。代替案は使っていない。二体の見かけの身長比は覗き窓からの 0.65 から 0.98 になり、目視でも同じ大きさに見える。
...
| 要素 | 現状 | 変更 | 根拠 |
|------|------|------|------|
| 水面の高さ | 1.504 | **0.80**(窓の腰 0.85 より 5 cm 低い) | 腰壁が水際の手前を隠すので、台の立ち上がりがどこからも見えない |
| 台 | 天面 1.5、z −3.2〜−7.8、脚あり | 中庭の床いっぱいの石のテラス(天端 0.82)に水盤をくり抜く形。脚は廃止 | 立ち上がりの面をなくし、水が窓のすぐ向こうから始まって見えるようにする |
| 岸の底面 | 床に接地 | **0.03 m 浮かせる** | 0 だと回廊の床と同一平面になり、深度が競って床に縞が出る |
| 手前の水際 | z −3.35 | **z −0.6**(テラスの手前端は壁の中、z −0.05) | 水面を手前まで引き寄せ、画面の中で帯にする |
| 生垣・植栽壁 | 全幅、高さ 1.58〜1.76 | 廃止 | これが水面を完全に遮っていた |
| 手前の景物 | z ≈ −3.4、中央にも | 左右の小島(x 絶対値 3.1〜4.7、z −1.4〜−2.7)に木 0.4〜0.55 | 中央を水面に譲る |
| 中景 | なし | 小島 2 つ(z ≈ −4.5、−5.2)に 0.18〜0.25 m の木 | 近・中・遠の大きさの勾配を作る |
| 奥の岸 | z −7.2〜−7.6 | z −7.55 に岸の帯。木は 0.12〜0.18 m | |
| 桟 | x = −2.13, 0, 2.13 | **x = ±1.6** の 2 本(3 枚のガラス) | 中央を塞がない |
| 窓ガラス | roughness 0.05 | **0.28**、envMapIntensity 0.35 | 庭の光が小さく強い玉になって映り込んでいた |
| 書き割り | 3 面をぴったり合わせる | 角で 0.4 m 重ねる | 隙間が縦の白い筋として見えていた |
| 庭の光 | スポットの狙いは台、強度 60、角度 π/3 | 狙いは庭の奥(0.5, 0.8, −6.5)、強度 46、角度 π/5 | 手前の岸を狙うと光だまりが白飛びして水面の手前が潰れる |
| 推奨視点 | pitch +0.02 | pitch **−0.15** | 水面の帯(水平から 5°〜23° 下)を画面中央寄りへ |
| 種明かしの人型・物差し | (−1.6, 0, −3.3) 等 | 水の中(−1.5, 0.8, −2.2)、物差し (1.6, 0.8, −2.0) | 頭頂 2.5 m が窓の上端 2.75 に収まる |

計算(実装後): 推奨視点から手前の水際への俯角は 23°、奥の水際は 5°。水面は垂直画角のうち約 18° を占める。この値は `waterViewBand()` が返し、単体テストで守っている。

#### 修正方針 B: 局所フォグ

three.js のフォグはシーン全体に掛かるので、**ジオラマ以外の材質でフォグを無効化**して局所化する。

- `scene.fog = new THREE.Fog(HAZE, 3.5, 13.5)`。カメラが窓から 1.3 m のとき、ジオラマはカメラから 1.9〜9.3 m にあり、この範囲に勾配が乗る。水面はフラグメントごとに霞むので、遠くの水面が霧に溶ける
- `applyFogScope(scene, scopes)`: ジオラマの `enclosure` 配下を除くすべての `Material` に `fog = false` を立て `needsUpdate` する。書き割り(空・遠山)と天井の空の面は `userData.noFog` で範囲の中でも除く(絵の階調を保つ)。展示の読み込みと足跡マークの追加が終わった後、`warmUpShaders` の前に 1 回実行する(`fog` はシェーダの define なので、先に確定させないと最初のフレームで再コンパイルが走る。足跡マークより先に呼ぶと、それだけ範囲から漏れる)
...
- `?dof=1` で試せるようにし、フェーズ R3 で A/B を行う(§9-2)

**A/B の結果(実 GPU、high ティア): 既定 OFF とする。** 同じ視点で `maxblur` 0.004 と 0.012 を試したが、霞が既に遠景を柔らかくしているため見た目がほとんど変わらなかった。強くすると今度はミニチュアの手がかりになり、狙いと逆に働く。毎フレームの追加パス(深度描画 + ぼかし)に見合わないので、既定では `BokehPass` を作らず、`?dof=1` を指定したときだけ遅延生成して差し込む形にした。
...
2. 彫刻の配置(mid/high): 立方体(本物)と球(鏡の中だけ)を**同じ位置に直立**させて置く。反射は鏡像として自然に上下逆になる。水面下の逆さの彫刻は置かない
   - 水盤を広げる(内法 2.2 → 2.9 m)。目の高さ 1.6 m・水面 1.0 m・正面 2.1 m から見ると、彫刻の頂部の反射は中心から 1.24 m のところに落ちる。旧寸法(内法の半分 1.1 m)では塔の上半分の反射が水面からはみ出して見えなかった
   - 反射カメラは水面の下(水盤の中)に入るので、水面より 5 cm 上に出ている内壁がすぐ目の前に来て視界を覆う。鏡の中では内壁を消す
   - フレネルの下限を 0.8 まで上げる。物理的には正面で 0.02 前後だが、それでは水面が暗い板になり、映り込みが読み取れない
3. low ティア: 反射描画を避け、現行方式(半透明の板 + 水面下の逆さの球)を残す
4. 種明かし(mid/high): 球の彫刻を主画面に半透明(0.6)で現し、`reflectionStrength` を 1 → 0.35 に落とす。主画面では青く染めて本物の白い立方体と見分けられるようにし、鏡の中では白のままにする。low は現行の `MaterialSwap` + `LightChange`
...
| R3 ポストプロセスと回廊 ✔ | `LoadContext.post`、`PostProcess` API、F3(§5.2)、F4(§5.3 A・B・C)。F3 と F4 を別 PR に分けた | R0 | S + L | 完了。F3 は実 GPU の high で稜線が消え、E2E も緑。F4 は水面が垂直画角の 18° を占め、霞はジオラマの中だけに効く。被写界深度は A/B の結果、既定 OFF(`?dof=1` で試せる) |
| R4 水面 ✔ | §5.5。`WaterSurface`、`InvertedPond` の再構成、low の代替、文言 | なし | L | 完了。E2E 2 件が緑(反射の間引きと種明かし)。実 GPU の high で、立方体の塔の下に球の塔が映ることを確認。実機 mid の fps は未計測(§10) |
| R5 エイムズの部屋 ✔ | §4.1。等距離視点と `CameraPath` | なし | M | 完了。単体 5 件が緑。E2E で、覗き窓からの見かけの身長比は 0.75 未満、等距離の視点では 0.98 |
| R6 仕上げ ✔ | 全展示のスクリーンショット撮り直し、`Docs/exhibits/` と README の最終確認 | R1〜R5 | S | 完了。単体 138 件と E2E 42 件が緑。F4 への `WaterSurface` の流用は見送った(霞で遠景が十分に柔らかく、反射を足す利得が小さいため) |
...
| 項目 | 内容 |
|------|------|
| 実機での fps | F7 の平面反射と F4 の霞を入れた後の実機(iOS Safari / Android Chrome)での計測は未実施。README の「実機での確認」の手順で mid ティア 30fps を確認する必要がある。反射テクスチャは mid で 256、視錐台と距離(12 m)で間引いてあるが、実測はしていない |
| F5 無限の井戸 | REFINE_PLAN.md に挙がっていないので触っていない。F7 の水面と同じくポータル描画を使っているため、両方が同時に見える位置での負荷は実機で見ておきたい |
| README の「現行」 | 「錯視として成立していない展示があります」の注記は残したままにしてある。8 項目は対応したが、成立しているかどうかの最終的な判断は作者に委ねる |
