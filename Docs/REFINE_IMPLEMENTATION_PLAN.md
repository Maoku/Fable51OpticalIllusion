...
| 項目 | 内容 |
|------|------|
| 実機での fps | F7 の平面反射と F4 の霞を入れた後の実機(iOS Safari / Android Chrome)での計測は未実施。README の「実機での確認」の手順で mid ティア 30fps を確認する必要がある |
| F5 無限の井戸 | REFINE_PLAN.md に挙がっていないので触っていない。F7 の水面と同じくポータル描画を使っているため、反射と重なったときの負荷は実機で見ておきたい |
