...
| 目 | なし | 任意: 眼窩の位置に暗い虹彩の円盤を置く。顔と認識されやすくなり、凸の解釈が強まる |
...
- 天井の高さを純関数 `stairCeilingHeight(x, z)` として `stairGeometry.ts` に定義する(踊り場: 段 + `headroom`。フライト: 低い端から高い端へ線形。1 周分、周回は `stairHeight` と同じ規則で選ぶ)
- `slab()` を「低い端と高い端の 2 点から傾きと中心を決める」形に書き換え、`sign` 引数を廃止する。建て方と検証が同じ関数を参照するようにする
- 修正後の期待値: 全フライトで天井高 ≥ 2.43 m。踊り場との接続部の段差 ≤ 0.05 m
